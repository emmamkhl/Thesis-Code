# Part 04 — Repeat Filtering & Metrics Extraction

## reason
Parse RepeatMasker `.out` files, apply biologically motivated filters (length and divergence thresholds; exclude low-complexity/simple repeats), and generate per-species summary tables and QC plots.

## filters applied
- Length > 200 bp  
- Divergence < 20%  
- Exclude: Simple_repeat, Low_complexity, Satellite  

## outputs
- `plots/` divergence histograms + top repeat class barplots per species  
- `tables/` per-species repeat summaries + combined tables across species  
- Cross-species summary + phylum comparison plot
...
cat("repeat filtering and metrics extraction\n")
...
parse_repeatmasker <- function(out_file) {
  ...
  tryCatch({
    ...
    if (ncol(data) >= 15) {
      colnames(data) <- c(
        "score", "div", "del", "ins", "query", "begin", "end",
...
phylum_map <- c(
  "hydra_vulgaris" = "cnidaria", "clytia_hemisphaerica" = "cnidaria", "nematostella_vectensis" = "cnidaria",
...
for (species in species_list) {
  ...
  if (!is.null(filtered_repeats) && nrow(filtered_repeats) > 0) {
    ...
    p1 <- ggplot(filtered_repeats, aes(x = div)) +
      ...
      labs(
        title = paste("divergence distribution -", gsub("_", " ", species)),
    ...
    p2 <- ggplot(class_summary, aes(x = reorder(class, n), y = n)) +
      ...
      coord_flip() +
      labs(
        title = paste("top repeat classes -", gsub("_", " ", species)),
...
if (length(all_results) > 0) {
  ...
  p3 <- ggplot(species_summary, aes(x = reorder(species, total_repeats), y = total_repeats / 1000, fill = phylum)) +
    ...
    coord_flip() +
    labs(
      title = "total filtered repeats by species",
